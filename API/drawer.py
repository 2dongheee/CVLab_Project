...
class Drawer(object):
    ...
    def draw(self, show_img, dets, labels, dists, fps, is_tracker=True):
        ...
        for idx in range(dets.shape[0]):
            ...
            if height < 50:
                ...
                dist_position = (dets[idx, 2] - int(2 * label_size[0][1]), dets[idx, 3]) 
                ...
                cv2.putText(show_img, label_str, bottom_left, self.fontFace, self.fontSize*0.5, self.whiteColor,
                            thickness=1)
                cv2.putText(show_img, str(dists[idx]), dist_position, cv2.FONT_HERSHEY_SIMPLEX, self.fontSize*0.5, (0, 255, 0),
                            thickness=1)
            ...
            ## distance
            Dist_str = "Dist_%d : %.2f"%(idx, dists[idx][0])
            cv2.putText(show_img, Dist_str, (int(0.01 * w), inform_size[0][1] + int(0.01 * h)+(idx*25)+40), self.fontFace, self.fontSize*0.7, (0,0,0), self.thickness)
